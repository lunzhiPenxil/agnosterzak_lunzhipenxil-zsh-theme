...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment yellow magenta "%{$fg_bold[magenta]%(!.%{%F{magenta}%}.)%}$USER@PC%{$fg_no_bold[magenta]%}"
  fi
}
...
prompt_time() {
  prompt_segment blue white "%{$fg_bold[white]%}%D{%Y/%m/%d - %a - %H:%M:%S}%{$fg_no_bold[white]%}"
}
...
prompt_cmd_exec_time() {
  [ $BULLETTRAIN_last_exec_duration -gt $BULLETTRAIN_EXEC_TIME_ELAPSED ] && prompt_segment white black "$(displaytime $BULLETTRAIN_last_exec_duration)"
}
